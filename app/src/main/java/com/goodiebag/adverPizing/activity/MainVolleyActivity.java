...
public class MainVolleyActivity extends AppCompatActivity implements Response.Listener<JSONArray>,
    ...
    public void onResponse(JSONArray response) {
        loading.dismiss();
        dataOrNot = true;
        String resp = "On_Response_Detected";
    ...
    private void parseData(Object response) {
        ...
        try {
            //Toast.makeText(getApplicationContext(), "Offers are available", Toast.LENGTH_LONG).show();
            JSONArray jArr = new JSONArray(rpiResponse);
            for(int i = 0; i < jArr.length(); i++){
                JSONObject arJ = jArr.getJSONObject(i);
                Item item = new Item();
                if (arJ.has("created_date"))
                    item.setDate("" + arJ.getString("created_date"));
                if (arJ.has("title"))
                    item.setName("" + arJ.getString("title"));
                if (arJ.has("description"))
                    item.setDescription("" + arJ.getString("description"));
                if (arJ.has("published_date"))
                    item.setLastDate("" + arJ.getString("published_date"));
                if (arJ.has("teacher"))
